...
$(document).ready(function(){
	...
	$("#btn_guardar").live("click", function(event){
		...
		if($("#txt_mensaje").val()==""){
			$.pnotify({
			    pnotify_title: 'Error!!',
    			pnotify_text: 'Debe indicar el mensaje',
    			pnotify_type: 'error',
    			pnotify_hide: true
				});
				$("#txt_mensaje").focus();
		...
		async: false,
        url: "operaciones.php",
        data: "opcion=1&txt_nombre="+$("#txt_nombre").val()+"&fecha_ini="+$("#fecha_ini").val()+"&fecha_fin="+$("#fecha_fin").val()+"&cmb_ini="+$("#cmb_ini").val()+"&cmb_fin="+$("#cmb_fin").val()+"&txt_total="+$("#txt_total").val()+"&txt_mensaje="+$("#txt_mensaje").val(),
 ...
 Programe su campa&ntilde;a</div>
  ...
  <table>
    ...
    <td><div class="Arial14Azul">
     <?=$_REQUEST['total_sms'];?>
      <input id="txt_total" name="txt_total" type="hidden" value="<?=$_REQUEST['total_sms'];?>" />
    </div></td></tr>
    <tr>
    <td><div class="Arial14Morado">Mensaje </div></td><td><textarea class=" textArea " name="txt_mensaje" id="txt_mensaje" cols="30" rows="3" maxlength="157"></textarea>
    </td>
    </tr>
