...
//$("#btn_buscar").click(function(event){
		...
		async: false,
        url: "operaciones/opr_empresas.php",
        data: "opcion=2&usuario="+$('#txt_empresa_buscar').val(),
        success: function(datos){
			//desconcateno el resultado la primera posicion me indica si fue exitoso
			if(datos=="error"){
				$.pnotify({
			    pnotify_title: 'La empresa no se encontro',
    			pnotify_text: '',
    			pnotify_type: 'info',
    			pnotify_hide: true
				});
...
$("#btn_guardar").click(function(event){
		...
		success: function(datos){
				alert(datos);
				$.pnotify({
			    pnotify_title: 'Empresa Modificado',
    			pnotify_text: '',
    			pnotify_type: 'info',
    			pnotify_hide: true
			});		
...
$("#btn_eliminar").click(function(event){
	...
	$.ajax({
		...
		async: false,
        url: "operaciones/opr_empresas.php",
        data: "opcion=4&txt_empresa_buscar="+$('#txt_empresa_buscar').val(),
        success: function(datos){
